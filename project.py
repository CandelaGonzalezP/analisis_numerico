...
def corregir_sintaxis(funcion_str):
    ...
    funcion_str = funcion_str.replace("sen", "sin").replace("e**x", "exp(x)")
...
def calcular_derivadas(funcion_str, x0, h):
    ...
    try:
        ...
        df_real = sp.lambdify(x, derivada_simbolica, modules=["numpy"])
        ...
        print("-"*50)
        print(f"Función derivada: {derivada_simbolica}")
        print(f"Función derivada evaluada en x = {x0}: {derivada_numerica}")
        print(f"Función derivada real en x = {x0}: {derivada_real}")
        print("-"*50)
